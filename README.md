...
1. **Full Website Audit** (`codebase-recon` ➔ `designer-me` ➔ `visual-qa-agent`)
2. **Mobile Reconstruction** (`codebase-recon` ➔ `designer-mobile` ➔ `visual-qa-agent`)
3. **Cinematic Motion** (`designer-me` ➔ `motion-architect` ➔ `interaction-qa-agent`)
4. **Route Transition** (`codebase-recon` ➔ `route-transition-designer` ➔ `interaction-qa-agent`)
5. **Release Gate** (`visual-qa-agent` ➔ `interaction-qa-agent` ➔ `deployment-verifier`)
